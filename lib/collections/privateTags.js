...
PrivateTags.schema = new SimpleSchema({
  name:     { type: String, max: parseInt(Meteor.settings.public.maxTagLength) },
  label:    { type: String, max: parseInt(Meteor.settings.public.maxTagLength) },
  userId:   { type: String },
...
validateTag = function (tag) {
  ...
  var maxTagLength = parseInt(Meteor.settings.public.maxTagLength);
...
Meteor.methods({
  ...
  privateTagInsert: function(tagLabel) {
    ...
    var options = { default: false };
    Meteor.call('privateTagInsertWithOptions', tagLabel, options);
  },

  /**
   * Inserts the tag into the PrivateTags collection (or db) with additional
   * options. If a tag by the same name already exists, the ID of the exising
   * tag is returned
   * @param string tagLabel The human readable form of the label
   * @return object The name of the newly created tag, or an existing tag name
   */
  privateTagInsertWithOptions: function(tagLabel, options) {
    ...
    // If default option is set, check for another default tag and get it
    if (options.default) {
      var defaultTag = PrivateTags.findOne({ default: true });
      if (defaultTag) {
        return defaultTag;
      }
    }
