...
Posts.deny({
  update: function(userId, post, fieldNames) {
    ...
    return (_.without(fieldNames,
              'bodyMessage', 'publicTags', 'privateTags').length > 0);
  }
});
...
Posts.deny({
  update: function(userId, post, fieldNames, modifier) {
    ...
    return errors.bodyMessage || errors.publicTags || errors.privateTags;
  }
});
...
validatePost = function (post) {
  ...
  if (!post.bodyMessage) {
    errors.bodyMessage =  "Please add some text";
  }
...
Meteor.methods({
  ...
  postInsert: function(postData) {
    ...
    if (errors.bodyMessage || errors.publicTags || errors.privateTags) {
      var msg = 'You must set a description and add at least one tag for your request';
  ...
  postUpdate: function(postData, postMetadata) {
    ...
    // Clean the post data and update, (validation happens automatically
  ...
  prayedFor: function(postId) {
    ...
    // search for the user's record in the post. If found, increment counter
    // Otherwise, add user and increment
    if (Posts.findOne({ _id: postId, precatis: this.userId } )) {
      // Increment the button click count
      affected = Posts.update({ _id: postId }, { $inc: { prayedCount: 1 } });
    } else {
      // Add user and increment the button click count
      affected = Posts.update({
        _id: postId,
        precatis: { $ne: this.userId }
      }, {
        $addToSet: { precatis: this.userId },
        $inc: { prayedCount: 1 }
      });
    }
    ...
    Meteor.call('logUserActivity', postId, Meteor.precariMethods.activity.PRAY);
  },
});
