...
Posts.deny({
  update: function(userId, post, fieldNames) {
    ...
    return (_.without(fieldNames,
              'prayer_request', 'title', 'tags', 'private').length > 0);
  }
});
...
Posts.deny({
  update: function(userId, post, fieldNames, modifier) {
    ...
    return errors.title || errors.prayer_request || errors.tags;
  }
});
...
Meteor.methods({
   ...
   *                generates an error.
   */
  postInsert: function(postData, postMetadata) {
    ...
    if (errors.title || errors.prayer_request || errors.tags) {
      throw new Meteor.Error('invalid-post', "You must set a title, description, and add tags for your request");
    }

    // Format the tags as private
    if (postMetadata.privateTags) {
      postData.tags = Meteor.call('formatTagsPrivate', postData.tags);
    }
  ...
  /**
   * Updates the post into the Posts collection (or db)
   * @param collection postData The data required for the post.
