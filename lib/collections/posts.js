...
Posts.schema = new SimpleSchema({
  title: {                  // The title of request/post
     type: String,
     label: "Title",
     max: parseInt(Meteor.settings.public.posts.maxTitleLength),
  },
...
validatePost = function (post) {
  ...
  var maxTitleLength = parseInt(Meteor.settings.public.posts.maxTitleLength);

  if (!post.title) {
    errors.title = "Please add a tile";
  } else if (post.title.length > maxTitleLength) {
    errors.title = "The title is limited to " + maxTitleLength + " characters.";
  }
