...
LatestPostsController = PostsListController.extend({
  ...
  subscriptions: function() {
    this.postsSub = Meteor.subscribe('publicPosts', this.findOptions());
  },
...
MyPostsController = PostsListController.extend({
  ...
  waitOn: function() {
    this.postsSub = [
      Meteor.subscribe('usersPrivateTags'),
...
PostsContainingTagController = PostsListController.extend({
  ...
  subscriptions: function() {
    ...
    // Get subscription from either the public or public tag
    this.postsSub = Meteor.subscribe
      ('postsFromTag', this.findOptions(), this.params.name);
  },
